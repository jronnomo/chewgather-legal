---
title: Terms of Service
permalink: /terms/
---

# Chewgether Terms of Service

**Effective date: July 1, 2026**

These Terms govern your use of the Chewgether app. By creating an account or using the app, you agree to them.

## 1. Who can use Chewgether

You must be at least 13 years old to use Chewgether. If you are under 18, you confirm you have your parent or guardian's permission.

## 2. Your account

You are responsible for your account and for keeping your password secure. Provide accurate information, and don't impersonate anyone. One account per person.

## 3. Community rules

Chewgether is for planning meals with people you actually want to eat with. You agree not to:

- Post content that is unlawful, hateful, harassing, threatening, sexually explicit, or discriminatory — including in your display name and plan titles;
- Harass, intimidate, or spam other users, including via plan invites or join requests;
- Impersonate any person, or misrepresent your affiliation with anyone;
- Attempt to access other users' accounts or data, probe or disrupt the service, or use the app for anything other than its intended purpose;
- Use the app to organize unlawful activity.

The app filters objectionable language automatically, and every user can **report** content or users and **block** anyone (blocks are mutual: neither side can contact the other).

## 4. Moderation and enforcement

We review reports and may remove content, restrict features, suspend, or terminate accounts that violate these rules — with or without notice, at our discretion. Report handling and moderation questions: **ggronnii@gmail.com** (we aim to act on reports within 24 hours).

## 5. Your content

You own the content you create (plan titles, photos, etc.). You grant us a limited, worldwide, royalty-free license to host, store, and display that content solely to operate the app (for example, showing your plan to the friends you invited). This license ends when you delete the content or your account.

## 6. Third-party content

Restaurant information (names, photos, hours, ratings) comes from Google Places and other providers. We don't guarantee its accuracy — hours change, kitchens close early. Reservations link out to third-party services; we are not a party to any booking.

## 7. Termination

You can stop using Chewgether at any time and delete your account in the app (Profile → Delete Account). We may suspend or terminate accounts that violate these Terms.

## 8. Disclaimers

The app is provided "as is" and "as available," without warranties of any kind, express or implied. We do not warrant that the app will be uninterrupted, error-free, or that any restaurant will live up to the group's expectations.

## 9. Limitation of liability

To the maximum extent permitted by law, Chewgether and its creator are not liable for any indirect, incidental, special, consequential, or punitive damages, or any loss of data, arising from your use of the app. Our total liability for any claim relating to the app is limited to $50 or the amount you paid us in the past 12 months, whichever is greater.

## 10. Changes to these Terms

We may update these Terms; if changes are material we will update the effective date above and notify you in the app. Continuing to use the app after changes take effect means you accept them.

## 11. Governing law

These Terms are governed by the laws of the Commonwealth of Virginia, USA, without regard to conflict-of-law rules.

## Contact

**ggronnii@gmail.com**

---
[Privacy Policy](/chewgether-legal/privacy/) · [Support](/chewgether-legal/)
